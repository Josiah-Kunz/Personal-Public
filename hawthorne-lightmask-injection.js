...
function shouldUpdate() {
    const now = Date.now();
    if (now - game.__lightMaskCache.lastUpdateTime < UPDATE_THROTTLE) {
        return false;
    }
    ...
    if (mapChanged || objectCountChanged || objectsChanged) {
        game.__lightMaskCache.lastUpdateTime = now;
...
function applyBlend(){
    ...
    // Only rebuild container if sprites actually changed
    const currentSpriteIds = allSprites.map(item => item.sprite.uid || item.sprite.id).join(',');
    if (game.__lightMaskCache.lastSpriteIds !== currentSpriteIds) {
        ...
        game.__lightMaskCache.lastSpriteIds = currentSpriteIds;
    }
}
